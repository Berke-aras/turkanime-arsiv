...
| 2026-09-23 | düzeltme | Uyum kuralı: yıl ve biçim (TV/Film/OVA) tutuyorsa bölüm sayısı farkı engel değil (Shingeki no Kyojin arşivde 31, AniList'te 25 bölüm diye şarkısız kalıyordu). `build-xray-ek.js --uyum` ile 235 kayda MAL kimliği, 210'una şarkı; şarkılı anime 4846 → 5056 |
| 2026-09-23 | güvenilirlik | Sibnet için ikinci çözücü: Netlify (`tka-sibnet.netlify.app`, `cozucu-netlify/`, aynı `api/sibnet.js`). İstemci (`js/cozum.js`) yükü video numarasına göre iki çözücüye bölüyor, yoğun/erişilemez olanı beklemeden atlayıp 1 dk sıranın sonuna koyuyor; ikisi de yoğunsa eskisi gibi 3 tur tekrar. `api/sibnet.js`: yanıt vermeyen Sibnet isteği bütçe sonunda kesilip 503 sayılıyor |
| 2026-09-23 | geri alma | Netlify çözücüsü şimdilik listeden çıkarıldı: yük testinden sonra Sibnet çıkış IP'sini 1,5 saati aşkın süre engelledi; ilk sıraya düştüğü videolarda ~8 sn gecikme yaratıyordu. Çoklu çözücü desteği ve süre sınırı istemcide kalıyor |
| 2026-09-23 | geri ekleme | Netlify çözücüsü listeye geri döndü: 16:00 UTC'de Netlify 200, Vercel 503 veriyordu. Sibnet'in engeli geçici ve IP'ye özel; iki çözücü birbirinin yedeği olarak işe yarıyor |
| 2026-09-23 | güvenilirlik | Vercel function'ı `iad1`'den `fra1`'e (Frankfurt) taşındı (`vercel.json` regions): iad1'in çıkış IP'leri binlerce Vercel müşterisiyle ortak ve Sibnet onları 403'lüyordu; Frankfurt hem ayrı IP havuzu hem Sibnet'e ve Türkiye'ye yakın |
